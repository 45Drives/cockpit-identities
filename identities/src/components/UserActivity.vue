	...
	setup(props) {
		...
		const getHistory = async () => {
			...
			try {
				...
				try {
					...
					pushNotification(new Notification("Error getting login history", errorStringHTML(state), 'error'));
					return;
				}
			} finally {
				processing.value--;
		...
		const saveCSV = async () => {
			try {
				...
				pushNotification(new Notification("Failed to download file", errorStringHTML(error), 'error'));
			}
		}
